...
if [ -n "$MISSING_PKGS" ]; then
    ...
    if [ -t 0 ]; then
        ...
        # Or just fail if not root?
        # User prompt said "ask the user", implying interactive.
        # If non-interactive, we can't ask. Let's assume we proceed if we can, or fail.
        # Let's assume 'No' for safety in non-interactive unless we are root.
        if [ "$EUID" -eq 0 ]; then
            REPLY="y"
        else
